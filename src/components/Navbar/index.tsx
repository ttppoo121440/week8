    ...
    <div>
      ...
      <div className="relative min-h-screen">
        <div className="px-6 py-4">
          <p className="py-3 ps-6">探索全部</p>
          <ul className="">
            {menu.map((item, index) => (
              <li key={item.title}>
                <a
                  href="#"
                  className="flex items-center justify-between rounded-xl py-3 pe-3 ps-6 hover:bg-[#0068FF] hover:font-semibold hover:text-white"
                >
                  <span>{item.title}</span>
                  <MdKeyboardArrowRight color="#909090" size={24} />
                </a>
              </li>
            ))}
          </ul>
